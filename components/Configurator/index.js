...
import { Spinner, AccountStorageQuery, AccountStorageMutation, Toast, Icon} from 'nr1'
...
export default class Configurator extends React.Component {
    ...
    */
        ...
        AccountStorageQuery.query({
            ...
            if(metaData) {
                ...
                if (hasDefault) {
                    ...
                    //debugger;
                    //this.saveToStorage(this.props.default); //removed as it can cause default config to be saved when data load fails
                }
                else {
    ...
    render() {
        ...
        const { schema, dataChangeHandler, documentId, storageCollectionId, buttonTitle, modalTitle, modalHelp, uiSchema ,loading } = this.props  
        ...
        if(configVisible) {
            ...
            if(dataLoaded) {
                ...
                let form = <Form schema={schema} 
                        ...
                        onError={log("errors")}
                        >
                        <button className="u-unstyledButton btn btn-info" type="submit"><Icon spacingType={[Icon.SPACING_TYPE.LARGE]} type={Icon.TYPE.HARDWARE_AND_SOFTWARE__SOFTWARE__UPSTREAM_CONNECTION} inline />Save Schemas</button></Form>
    
                returnVal=<>                    
                    <div className="ConfiguratorForm">
                        <span>Manage your synthetic configuration schemas here. For each synthetic configuration you must supply a name, unique ID and JSON schema for the form fields.  For help with schema definitions visit <a href="https://rjsf-team.github.io/react-jsonschema-form/">JSON Schema Playground</a><br /><br /></span>
                        {form}
                    </div>
         ...
         return <div >
            ...
            <div style={{width:"50%",marginTop:"2em", marginLeft:"2em"}}>{returnVal}</div>
           </div>
    }
}
